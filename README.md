...
A very crude client-side javascript browser patch for the Teams web-interface,
which reduces the margins / padding and width restrictions. It effectively makes the browser window
dictate the content width, and removes a lot of empty vertical space, which increases the amount of
content that fits on one screen.

Unfortunately it looks aesthetically...questionable :), but then again, I didn't try very hard. It's also hard
to not break the like buttons and the reply button at the same time.

Anyway: copy-paste and give it a try if you dare:
